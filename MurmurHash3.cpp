                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    double seed;            /* seed */
    double *out;          /* output hash */

    /* check for proper number of arguments */
    if( nrhs != 2 ) {
        mexErrMsgIdAndTxt("MyToolbox:MurmurHash3:nrhs", "Two inputs required.");
    }
    if( nlhs != 1 ) {
        mexErrMsgIdAndTxt("MyToolbox:MurmurHash3:nlhs", "One output required.");
    }
    /* make sure the first input argument is a String */
    if( !mxIsChar(prhs[0]) ) {
        mexErrMsgIdAndTxt("MyToolbox:MurmurHash3:notString", "Input key must be a String.");
    }
    
    /* make sure the second input argument is type double */
    if( !mxIsDouble(prhs[1]) || 
         mxGetNumberOfElements(prhs[1]) !=1 ) {
        mexErrMsgIdAndTxt("MyToolbox:MurmurHash3:notDouble", "Input seed must be type double.");
    }
